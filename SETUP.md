...
7c. **Install the push guard GLOBALLY** (every bind; it protects every project on the machine):
   ...
   - **Ask before the global auto-approve.** The guard itself only ever blocks, so it installs without asking. The auto-approve widens what runs without a prompt, so ask: *"Let every `git push` run without a prompt in EVERY project on this machine, including ones not set up with these templates? The push guard blocks every force push whose flag is in the command's text. These would run without a prompt: a force flag the shell builds at run time (a variable, command-substitution output, brace expansion, `xargs`, string concatenation) or hidden in git config or an alias, whether already on disk or set in the same command through `GIT_CONFIG_*` variables or `--config-env`; git named through a variable (`G=git; $G push -f`); remote branch and tag deletions, and a deleted branch pushed again; a push run by another program (`python -c`, `node -e`, `make`, `gh api`); a push after a spaced git option value ending in a git command name (`git -C "My notes" push -f`), or split from `git` by a `;`, `|` or `&` inside a quoted string that spans lines; and any push in a command over 100,000 characters. [y/N]"* On "y", add `"Bash(git push:*)"` to `permissions.allow` in `~/.claude/settings.json`, in the same atomic write as the hook entry. On anything else, leave it out, and tell the user that pushes other than a bare `git push` will ask for approval. If `"Bash(git push:*)"` is already in `permissions.allow` (the user may have written it before this setup), ask instead, naming the same pushes as the question above: *"Your global settings already let every `git push` run without a prompt in every project. The push guard now blocks the force pushes it can read. These would still run without a prompt: <the list from the question above>. Keep that? [Y/n]"*, and remove it only on "n". The project's deny rules take precedence either way.
