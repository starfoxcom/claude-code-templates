...
7c. **Install the push guard GLOBALLY** (offered on every bind; it protects every project on the machine):
   ...
   - **Ask before installing**, since it changes settings for every project: *"Install the push guard in `~/.claude`? It blocks force pushes in every project and lets safe pushes (`git push`, `-u`, `--tags`, `--force-with-lease`) run without a prompt everywhere. [Y/n]"* On "n", run the skip branch and tell the user that pushes other than a bare `git push` will ask for approval.
   - **Skip branch** (no Python, or the user declined): using Phase 7a's backup, shape normalization and atomic-write rules, remove any `hooks.PreToolUse` entry whose `args` mention `push-guard.py` and remove `"Bash(git push:*)"` from `permissions.allow` in `~/.claude/settings.json`. A re-run after the interpreter was removed then leaves no dead guard behind an auto-approve. Stop there; the steps below run only when installing.
   - **Tell the user** after installing: *"The push guard runs on `<python>`. If that interpreter is removed, the guard stops running while pushes stay auto-approved; re-run setup, or delete `Bash(git push:*)` from `~/.claude/settings.json`."* Claude Code reports a hook that cannot start as a non-blocking error, so this is the only other warning they get.
