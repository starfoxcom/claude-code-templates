...
7c. **Install the push guard GLOBALLY** (every bind; it protects every project on the machine):
   ...
   - **Find Python 3.8+ outside any virtual environment.** Run `python -c "import os, sys; assert sys.version_info >= (3, 8) and sys.prefix == sys.base_prefix; assert not (os.path.isdir(os.path.join(sys.prefix, 'conda-meta')) and os.environ.get('CONDA_DEFAULT_ENV', 'base') != 'base'); print(sys.executable)"`, then the same with `python3`, then `py -3`, and keep the first absolute path printed. The checks reject a project's virtualenv or a non-base conda environment: the guard serves every project, so it must not depend on one project's environment. When a candidate fails only because it runs inside an environment (a project's venv or conda env is often active in this shell), also try the interpreter that environment came from, with the same check: `<base>/bin/python3` or `<base>/python.exe`, where `<base>` is what that candidate prints for `import sys; print(sys.base_prefix)`, and for conda `$CONDA_PYTHON_EXE`. Then try `/usr/bin/python3`, `/usr/local/bin/python3` and `/opt/homebrew/bin/python3`. If still none qualifies:
     ...
     - **Only interpreters inside an environment were found:** change nothing (an existing guard stays installed) and tell the user: *"Only Python interpreters inside a virtual or conda environment were found, so the push guard was not installed or changed. Deactivate the environment and re-run setup to install it."*
