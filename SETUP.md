...
7c. **Install the push guard GLOBALLY** (every bind; it protects every project on the machine):
   ...
   - **Find Python 3.8+ outside any virtual environment.** Run `python -c "import os, sys; assert sys.version_info >= (3, 8) and sys.prefix == sys.base_prefix; up = os.path.dirname(sys.prefix); assert not (os.path.basename(up) == 'envs' and os.path.isdir(os.path.join(os.path.dirname(up), 'conda-meta'))); print(sys.executable)"`, then the same with `python3`, `py -3`, and `python3.13` down to `python3.8`, and keep the first absolute path printed. The checks reject a project's virtualenv and a conda environment under `<conda root>/envs/` (the conda root itself passes): the guard serves every project, so it must not depend on one project's environment. When a candidate fails only because it runs inside an environment (a project's venv or conda env is often active in this shell), also try the interpreter that environment came from, with the same check: `<base>/bin/python3` or `<base>/python.exe`, where `<base>` is what that candidate prints for `import sys; print(sys.base_prefix)`, and for conda `$CONDA_PYTHON_EXE`. Then try `/usr/bin/python3`, `/usr/local/bin/python3` and `/opt/homebrew/bin/python3`. If still none qualifies:
     ...
     - **Only interpreters inside an environment were found:** if `~/.claude/settings.json` has a `push-guard.py` entry whose `command` no longer runs (`<command> -c "print(1)"` fails), run the **skip branch** and say the old guard's interpreter is gone. Otherwise change nothing (a working guard stays installed). Either way tell the user: *"Only Python interpreters inside a virtual or conda environment were found, so the push guard was not installed or updated. Deactivate the environment and re-run setup to install it."*
   - **Skip branch** (no Python found): using Phase 7a's backup, shape normalization and atomic-write rules, remove any `hooks.PreToolUse` entry whose `args` mention `push-guard.py` and remove `"Bash(git push:*)"` from `permissions.allow` in `~/.claude/settings.json`, then delete `~/.claude/hooks/push-guard.py` if present. A re-run after the interpreter was removed then leaves no dead guard behind an auto-approve. Stop there; the steps below run only when installing.
   ...
   - **Ask before the global auto-approve.** The guard itself only ever adds prompts or blocks, so it installs without asking. The auto-approve widens what runs without a prompt, so ask: *"Let safe pushes (`git push`, `-u`, `--tags`, `--delete`, `--force-with-lease` and plain branch names) run without a prompt in EVERY project on this machine, including ones not set up with these templates? Force pushes stay blocked. [y/N]"* On "y", add `"Bash(git push:*)"` to `permissions.allow` in `~/.claude/settings.json` (skip it if already there), in the same atomic write as the hook entry. On anything else, remove it if present, and tell the user that pushes other than a bare `git push` will ask for approval. The project's deny rules take precedence either way.
