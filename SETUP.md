...
7c. **Install the push guard GLOBALLY** (every bind; it protects every project on the machine):
   ...
   - **Ask before the global auto-approve.** The guard itself only ever blocks, so it installs without asking. The auto-approve widens what runs without a prompt, so ask: *"Let every `git push` run without a prompt in EVERY project on this machine, including ones not set up with these templates? The push guard blocks the force pushes it can read. These would run without a prompt: a force flag built at run time (a variable, command-substitution output, brace expansion, `xargs`, text piped into a shell) or hidden in an alias or git config; a shell string run by a program other than a common wrapper such as `sudo`, `env` or `timeout` (for example `find -exec sh -c`); remote branch and tag deletions, and a deleted branch pushed again under another name such as `HEAD` or with the delete or the push inside `$(...)`, `sh -c`, `eval` or an alias; any push in a command over 20,000 characters; a push nested more than 8 levels deep, or inside a nested string, alias or inner substitution of a command that takes more than 3 seconds to read in full; and, in a command the guard cannot read to the end (an unclosed heredoc or another `<<` it cannot place, unbalanced quotes or parentheses, a PowerShell here-string or block comment), any push from the unreadable point on or nested anywhere in it, or in PowerShell any push at all in a command with non-ASCII characters, a vertical tab or form feed, or `--%`. [y/N]"* On "y", add `"Bash(git push:*)"` to `permissions.allow` in `~/.claude/settings.json`, in the same atomic write as the hook entry. On anything else, leave it out, and tell the user that pushes other than a bare `git push` will ask for approval. If `"Bash(git push:*)"` is already in `permissions.allow` (the user may have written it before this setup), ask instead, naming the same pushes as the question above: *"Your global settings already let every `git push` run without a prompt in every project. The push guard now blocks the force pushes it can read. These would still run without a prompt: <the list from the question above>. Keep that? [Y/n]"*, and remove it only on "n". The project's deny rules take precedence either way.
