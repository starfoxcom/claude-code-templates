...
- The push guard plan (always; Phase 7c): the file it copies to `~/.claude/hooks/push-guard.py`, the `PreToolUse` hook it registers in `~/.claude/settings.json` with the interpreter found (or "skipped: no Python 3.8+", with the entry, allow rule and file the skip branch would remove, or "unchanged: only environment interpreters found"), that the hook then runs on every Bash and PowerShell call in every project on the machine, that it never asks and blocks any command whose text holds a force push, including text that only mentions one (a commit message, a PR body, a search pattern) and, rarely, a plain push chained after a quoted string that spans lines or in a Bash command with a `\` ending a line when a force word follows it anywhere, telling Claude to pass such text in a file instead (or to put such a command on one line, or run the push as its own command), and that setup will ask separately whether pushes may skip the prompt everywhere (or, when `Bash(git push:*)` is already in the global allow list, whether to keep it)
...
7c. **Install the push guard GLOBALLY** (hook location "home", or on request; it protects every project on the machine):
   ...
   - **Why:** the project's deny rules match command text, so a force push spelled as a bundle of short flags (`git push -qf`) slips past them. The hook reads the command's text with quoting removed and blocks a `git push` followed in the same statement by any force flag, `--mirror` or `+` refspec, wherever it appears (inside `$(...)`, `sh -c '...'`, `eval`, a heredoc or a comment too), while `--force-with-lease` stays allowed. It never asks. Because it reads text, a command that only mentions a force push (a commit message, a script, a search pattern) is blocked too, and so, rarely, is a plain push chained after a quoted string that spans lines, or in a Bash command with a `\` ending a line when a force word follows it anywhere (`git push origin main \` then `&& rm -rf build`); the block message says to pass such text in a file, or to put such a command on one line or run the push as its own command, so this costs no prompt. No command guard can see a git alias that pushes with force, or a forcing `remote.<name>.push` or `mirror` setting, that is already in git's config when a plain `git push` runs, or a push built inside a program that an allow rule lets run other commands (the template's `python -c` and `node -e` rules, and runner stack commands such as `bundle exec`, `uv`, `poetry`, `npx`, `docker run` or `make`), or a branch update through `gh api` (GitHub's REST API); remove those rules if that matters for the project. Installed by this step, it lives in `~/.claude` and covers every project on the machine.
   - **Find Python 3.8+ outside any virtual environment.** If `~/.claude/settings.json` already has a `push-guard.py` entry, try its `command` first with the check below, so a working guard is refreshed even when its interpreter is not on `PATH`. Run `python -c "import os, sys; assert sys.version_info >= (3, 8) and sys.prefix == sys.base_prefix; up = os.path.dirname(sys.prefix); assert not (os.path.basename(up) == 'envs' and os.path.isdir(os.path.join(os.path.dirname(up), 'conda-meta'))); print(sys.executable)"`, then the same with `python3`, `py -3`, and `python3.13` down to `python3.8`, and keep the first absolute path printed. The checks reject a project's virtualenv and a conda environment under `<conda root>/envs/` (the conda root itself passes): the guard serves every project, so it must not depend on one project's environment. When a candidate fails only because it runs inside an environment (a project's venv or conda env is often active in this shell), also try the interpreter that environment came from, with the same check: `<base>/bin/python3` or `<base>/python.exe`, where `<base>` is what that candidate prints for `import sys; print(sys.base_prefix)`, and for conda `$CONDA_PYTHON_EXE`. Then try `/usr/bin/python3`, `/usr/local/bin/python3` and `/opt/homebrew/bin/python3`. If still none qualifies:
     - **No Python 3.8+ ran at all:** run the **skip branch** below and tell the user: *"No system Python 3.8+ found, so the push guard is not installed. Pushes other than a bare `git push` will ask for approval; install Python and re-run setup to add the guard."*
     - **Only interpreters inside an environment were found:** if `~/.claude/settings.json` has a `push-guard.py` entry, its interpreter failed the check above (the first candidate), so run the **skip branch** and say the old guard was removed. Otherwise change nothing. Either way tell the user: *"Only Python interpreters inside a virtual or conda environment were found, so the push guard was not installed or updated. Deactivate the environment and re-run setup to install it."*
   - **Skip branch** (no Python found): using Phase 7a's backup, shape normalization and atomic-write rules, remove any `hooks.PreToolUse` entry whose `args` mention `push-guard.py`; when there was such an entry, also remove `"Bash(git push:*)"` from `permissions.allow` in `~/.claude/settings.json` and say so (without an entry, a rule the user wrote stays); then delete `~/.claude/hooks/push-guard.py` if present. A re-run after the interpreter was removed then leaves no dead guard behind an auto-approve. Stop there; the steps below run only when installing.
   ...
   - **Ask before the global auto-approve.** The guard itself only ever blocks, so it installs without asking. The auto-approve widens what runs without a prompt, so ask: *"Let every `git push` run without a prompt in EVERY project on this machine, including ones not set up with these templates? The push guard is a safety net for force pushes written the ordinary way, not a sandbox against a command built to slip past it. These would run without a prompt: a force flag the shell builds at run time (a variable, command-substitution output, `xargs`, brace expansion, text piped into a shell or `iex`, string concatenation, PowerShell's `-f` format operator, PowerShell's `--%`); git config or an alias that forces a later plain push, whether already on disk or set in the same command through `GIT_CONFIG_*` variables or `--config-env`; git reached by another name (a variable such as `G=git; $G push -f`, a shell alias or `Set-Alias`, `hash -p`, a glob such as `gi[t]`); remote branch and tag deletions, and a deleted branch pushed again; a push run by another program (`python -c`, `node -e`, `make`, `gh api`, `git fetch --upload-pack`); a push after a git option value with spaces in it, one of whose later words is a git command name (`git -C "My notes dir" push -f`); shell syntax the guard reads differently from the shell where that cuts a push off from `git` or its force flag (quotes paired differently, such as a quoted string that spans lines or a `$(` inside single quotes, or unbalanced parentheses inside a substitution); and any push in a command over 100,000 characters. [y/N]"* On "y", add `"Bash(git push:*)"` to `permissions.allow` in `~/.claude/settings.json`, in the same atomic write as the hook entry. On anything else, leave it out, and tell the user that pushes other than a bare `git push` will ask for approval. If `"Bash(git push:*)"` is already in `permissions.allow` (the user may have written it before this setup), ask instead, naming the same pushes as the question above: *"Your global settings already let every `git push` run without a prompt in every project. The push guard now blocks the force pushes it can read. These would still run without a prompt: <the list from the question above>. Keep that? [Y/n]"*, and remove it only on "n". The project's deny rules take precedence either way.
   - **Tell the user** after installing, when the auto-approve was added: *"The push guard runs on `<python>`. If that interpreter is removed, the guard stops running while pushes stay auto-approved; re-run setup (it removes the auto-approve when no Python is found), or delete `Bash(git push:*)` from `~/.claude/settings.json`."* Claude Code reports a hook that cannot start as a non-blocking error, so this is the only other warning they get.
   - **Verify** by piping `{"tool_name": "Bash", "tool_input": {"command": "git push -qf origin main"}}` into the registered command and arguments: it must exit 2. The same input with `--force-with-lease` must exit 0 with no output.

7d. **Install the attribution guard GLOBALLY** (hook location "home" with the attribution guard on; skip it otherwise, and see 7c's first bullet):
   - **Why:** the guard denies a git or gh write whose text, or a message file it names, carries AI attribution: an AI co-author trailer, a "generated with" line, a session link or the robot emoji. Plain mentions of a tool pass. It is a safety net for text written the ordinary way, not a sandbox; its header lists what it cannot see.
   - Use the Python path 7c found. If 7c found none, skip this step and say the attribution guard was not installed either.
   - **Copy** `claude-code-templates/_core/global-template/hooks/no-ai-attribution.py` to `~/.claude/hooks/no-ai-attribution.py` (atomic write, as in Phase 7a), replacing any older copy.
   - **Register it** in `~/.claude/settings.json` with 7c's entry shape, `no-ai-attribution.py` in place of `push-guard.py`, after removing any entry whose `args` mention `no-ai-attribution.py`. It needs no permission rule: it only ever denies.
   - In the same atomic write, set `"attribution": {"commit": "", "pr": "", "sessionUrl": false}` in `~/.claude/settings.json`, so the harness adds no trailers of its own.
   - **Verify** by piping a `git commit -m` whose message ends with an AI co-author trailer into the registered command: it must print a JSON `deny` decision and exit 0. `git commit -m "docs: x"` must exit 0 with no output.
...
12. **Report a summary** to the user: files created, sections stripped, files deleted/skipped, files scaffolded, the commit SHA, the `~/.claude/` merge result, and the push guard result (installed with which interpreter, or skipped and what was cleaned up, and whether pushes now skip the prompt in every project).
