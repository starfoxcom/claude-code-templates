...
- **Task tracking is mandatory for 3+ step sessions.** Use the `TaskCreate` / `TaskUpdate` / `TaskList` / `TaskGet` tools the harness exposes. Enumerate the planned steps with `TaskCreate` at session start (use `addBlockedBy` for dependencies). `TaskUpdate(taskId, status='in_progress')` BEFORE starting; `TaskUpdate(taskId, status='completed')` immediately when done — don't batch. New follow-ups discovered mid-session get their own `TaskCreate` call. The task list is the in-session source of truth and the basis for the session-close DoD audit. Single-task or trivial conversational work doesn't need it; the threshold is 3+ discrete items.
...
The routine-review and deep-review workflows live at `.github/workflows/claude-code-review.yml` and `.github/workflows/claude.yml`. The single required status check `Evaluate review outcome` (in `claude-code-review.yml`) gates the routine tier and resolves on every PR — including docs-only and no-comment diffs. The deep tier's `Evaluate deep-tier verdict` step (in `claude.yml`) exits 1 on Opus 🔴, surfacing a blocking signal in the `Claude On-Demand` check run; it is advisory — not a required check — but a maintainer must read any 🔴 finding and confirm it is not applicable before admin-merging.
