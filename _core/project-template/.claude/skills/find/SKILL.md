...
The single approved entry point for code research in this project. Replaces ad-hoc Grep/Glob/raw `grep`/`rg` reflexes that the tokensave-first hook now blocks.
...
## The sequence (follow in order, stop at first useful result)

1. **`tokensave_search <name>`** — symbol-by-name lookup. Returns matches with file:line and short context. Fastest.
2. **`tokensave_context <natural-language query>`** — broader exploration. Returns related symbols + relationships. Use when the symbol name is unknown or fuzzy. Add `keywords: [...]` for synonyms.
...
Allowed ONLY if:
- You've tried tokensave with 2+ keyword variants and got nothing usable.
- You're searching non-code content (markdown, binaries, `.gitignored` files).
- `tokensave_status` returns `unavailable` for the scope you need.

If your case fits one of those, the `tokensave-first.py` hook (at `~/.claude/hooks/tokensave-first.py`, installed globally) is the gatekeeper:
...
When this skill produces the answer, cite the tokensave call(s) you used:
> Located `X` via `tokensave_search "X"` at `path/to/file.ext:42`.

The citation lets the user verify the path you took. It's also what the session-close adherence metric counts.

## Why this exists

Tokensave indexes the codebase as a symbol graph; queries return seconds of work in tens of tokens. Plain Grep can require reading 5–20 candidate files at thousands of tokens each before you locate the right one. Across sessions, this compounds — the project's session telemetry showed tokensave hit-rate trending DOWN while Bash usage grew. The hook + this skill exist to invert that.
