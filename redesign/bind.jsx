...
function BindFolio() {
  ...
  const [project, setProject] = bUseState({
    ...
    main_branch: "",  // empty = mirror default_branch; explicit value overrides
  ...
  function buildManifest() {
    return {
      ...
      project: showProject ? {
        ...project,
        // main_branch is optional: when blank it mirrors default_branch.
        main_branch: project.main_branch || project.default_branch,
      } : {
        discovery: true,
        ...
        <section className="step" ref={el => stepRefs.current.project = el}>
          ...
          <div className="card pad" style={{ background: "var(--paper-card)" }}>
            <div className="proj-grid">
              ...
              <Field label="Default branch" hint="Where shipped/released versions live. Usually main.">
                <input className="input" placeholder="main"
                  value={project.default_branch}
                  onChange={e => setProject(p => ({ ...p, default_branch: e.target.value }))} />
              </Field>
              <Field label="Developer branch" hint="Where day-to-day work lands. Gitflow: develop. Trunk: same as default.">
                <input className="input" placeholder="develop"
                  value={project.developer_branch}
                  onChange={e => setProject(p => ({ ...p, developer_branch: e.target.value }))} />
              </Field>
              <Field label="Production branch"
                hint="Optional. Defaults to the default branch above. Only set if you keep a separate production line — used for audit-trail / strict branch protection.">
                <input className="input" placeholder={project.default_branch || "main"}
              ...
              <Field label="Branching model" hint="Gitflow = separate default + developer branches. Trunk = single branch.">
                <select className="select"
...
function advancedFilledCount(p) {
  const n = ["description", "main_branch", "branching_model", "license_holder", "timezone", "code_language"]
    .filter(k => {
      if (k === "main_branch") return p.main_branch && p.main_branch !== p.default_branch;
...
2. Parse \`./claude-code-templates/manifest.json\`. Required fields:
   \`bundle\`, \`project.name\`, \`project.repo_url\`, \`project.default_branch\`,
   \`toggles\`. Missing fields → run a short interview to fill them.
...
- Substituted placeholders (\`{{PROJECT_NAME}}\`, \`{{REPO_URL}}\`,
  \`{{MAIN_BRANCH}}\`, \`{{DEFAULT_BRANCH}}\`, etc.)
