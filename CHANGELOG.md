...
- **Workflow-only PRs from the maintainer's own sessions merge on two local reviews.** `review-tiers.md` and `token-efficiency.md` no longer make the maintainer read every PR that changes `.github/workflows/`. No AI reviews such a PR in CI, so two local subagent reviews (one on the exact routine-review prompt, one as the deep tier) run before the push, and the PR merges once both give 🟢. A workflow-only PR from anyone else still merges only after the maintainer has read its whole diff.
