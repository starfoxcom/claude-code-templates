...
- **Deep-review CI model bumped Opus 4.7 → 4.8** (PR #111 → `main`, #112 cascade → `develop`). `.github/workflows/claude.yml`'s on-demand deep review now runs `claude-opus-4-8`; the canonical `claude.yml.template` matched in lockstep. Adopted as a **watched trial** per the repo's pinned-incumbent discipline — 4.8 remediates 4.7's long-context-retrieval + multi-step-instruction-following regressions and is ~4× less likely to let code bugs pass unremarked, at the same price. Routine review stays `claude-sonnet-4-6` (no Sonnet 4.8). Monitor the next several deep-review verdicts; revert (one-line) if it degrades.
- **Canonical review-workflow templates de-versioned** (PRs #113 + #115 → `main`, #116 → `develop`). The deep/routine `--model` in `_core/project-template/.github/workflows/claude.yml.template` and `claude-code-review.yml.template` is now a `{{REVIEW_DEEP_MODEL}}` / `{{REVIEW_ROUTINE_MODEL}}` placeholder (default-only, no bind-UI field), and the model version is dropped from each template's header comment + job display name — so a future model release edits **zero** files under `_core/project-template/`. SETUP.md Phase 3 resolves both to deliberate stable defaults (`claude-opus-4-8` deep / `claude-sonnet-4-6` routine); the canonical placeholder allowlist in `claude-code-review.yml` registers the two tokens so the diff-scoped scan recognizes them. Same staleness fix the session-start skill got, applied to the workflow templates. Live workflows keep concrete `--model` literals (resolved mirror).
...
- **`{{STACK_COMMANDS_ALLOWLIST}}` registered in the canonical placeholder allowlist** (PR #113). The placeholder is used in `_core/project-template/.claude/settings.local.json.template` but was never listed in `claude-code-review.yml`'s `CANONICAL` set — a latent `UNKNOWN_PLACEHOLDER` that would have tripped the canonical scan the moment that file landed in a reviewed diff. Registered alongside the new review-model tokens.
