...
Targeted for **v1.4.0** (minor bump — new feature, no break) once cut. These are the develop-only changes that landed after the v1.3.0 cut. Where a workflow/model item below had a **live-workflow** half, that half shipped in v1.3.0 (see [v1.3.0] § "Also shipped — CI review-gate hardening"); what remains here is the **canonical-template + bind** counterpart.
