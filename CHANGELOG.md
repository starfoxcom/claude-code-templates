...
- **Global push guard** (PR #158). `_core/global-template/hooks/push-guard.py` is a `PreToolUse` hook for Bash and PowerShell that SETUP.md Phase 7c installs in `~/.claude` on every bind (with the Python interpreter it finds, in exec form). It blocks certain force pushes that text rules cannot see (`-f` inside a short-flag bundle such as `-qf`, `--mirror`, `+` refspecs), keeps `--force-with-lease` allowed, lets only a short allow-list of plain safe pushes pass silently, and asks for everything else. Setup lists the guard in the plan it shows before applying, and asks `[y/N]` before adding a global `Bash(git push:*)` allow that lets those safe pushes skip the prompt in every project.
