...
- **Workflow-only PRs from the maintainer's own sessions merge on two local reviews.** `review-tiers.md` no longer makes the maintainer read every PR that changes `.github/workflows/`, and `token-efficiency.md` and the review-workflow template's header comment point to it. No AI reviews such a PR in CI. When every commit in it was written in the maintainer's own sessions, two local subagent reviews run on the exact head commit that will merge: one on the routine-review prompt taken from the base branch's copy of the workflow, never the PR's own, and one as the deep tier. The PR merges once both give 🟢, and any new commit re-runs both. Anything else, including a maintainer-opened PR that carries someone else's commits, still merges only after the maintainer has read its whole diff.
