...
- **Global push guard** (PR #158). `_core/global-template/hooks/push-guard.py` is a `PreToolUse` hook for Bash and PowerShell that SETUP.md Phase 7c installs in `~/.claude` on every bind (with the Python interpreter it finds, in exec form). It blocks force pushes that text rules cannot see (`-f` inside a short-flag bundle such as `-qf`, `--mirror`, `+` refspecs, forcing `-c` settings and one-off aliases, a force push inside `$(...)`, `sh -c` or `eval`, and a branch deleted and pushed again in one command) and keeps `--force-with-lease` allowed. It never asks: every command it cannot read for certain runs as it would without the guard, so it adds no prompts to normal work. It always answers well inside the hook timeout: a quick first reading of every plain statement blocks a plain force push even when the full reading of a crafted command runs out of time. Setup lists the guard in the plan it shows before applying, and asks `[y/N]` before adding a global `Bash(git push:*)` allow that lets pushes skip the prompt in every project.
